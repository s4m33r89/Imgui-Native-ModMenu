...
bool Tools::ProcessRead(void *address, void *buffer, size_t size, bool write = false) {
    struct iovec local[1];
    struct iovec remote[1];
...
void Tools::writeValue(uintptr_t address, void *value, ssize_t size) {
    struct iovec local[1];
    struct iovec remote[1];
    ...
    remote[0].iov_len = size;
    syscall(process_vm_writev_syscall, target_pid, local, 1, remote, 1, 0);
}


bool Tools::PVM_ReadAddr(void *addr, void *buffer, size_t length) {
    return Tools::ProcessRead(addr, buffer, length, false);
}

bool Tools::PVM_WriteAddr(void *addr, void *buffer, size_t length) {
    return Tools::ProcessRead(addr, buffer, length, true);
}
bool Tools::IsPtrValid(void *addr) {
    uintptr_t p = -1;
    return Tools::ProcessRead(addr, &p, sizeof(uintptr_t)) && p != -1;
}
